...
- 이미지로만 구성된 스캔 PDF는 OCR 기능이 없어 처리하기 어렵습니다.
- 현재는 한 세션에서 하나의 PDF를 중심으로 검색하며, 새 PDF 업로드 시 기존 FAISS 인덱스가 교체됩니다.
- 사용자 로그인 및 사용자별 학습 기록 관리
- 로그인 기능이 없어 사용자 계정이 아닌 세션 ID를 기준으로 학습 기록을 구분합니다.
...
- 여러 PDF를 하나의 세션에서 함께 검색하도록 개선
- 사용자 로그인 및 사용자별 학습 기록 관리
- 사용자 로그인 및 사용자별 퀴즈·오답 기록 관리
